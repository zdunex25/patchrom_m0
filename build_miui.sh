...
cd m0
mkdir AntiSpam Backup BugReport Calculator Calendar CalendarProvider CloudService Contacts ContactsProvider DeskClock DownloadProvider DownloadProviderUi FileExplorer MiuiCompass MiuiGallery MiuiKeyguard MiuiVideo MiWallpaper NetworkAssistant2 Notes PackageInstaller PaymentService Provision SoundRecorder TelephonyProvider Transfer VpnDialogs Weather WeatherProvider XiaomiServiceFramework YellowPage temp
cd temp
...
unzip -q out/temp/system/app/ContactsProvider.apk "resources.arsc"
cp -f ../miui/XHDPI/system/app/ContactsProvider.apk out/temp/system/app/ContactsProvider.apk
zip out/temp/system/app/ContactsProvider.apk -q resources.arsc
rm -f "resources.arsc"
...
cp ../../stockrom/file_contexts file_contexts
zip -q -r "../../unsigned-miuigalaxy-v5-sgs3-$version-4.4.zip" 'data' 'META-INF' 'system' 'boot.img' 'file_contexts'
cd ../..
fi
. ../build/envsetup.sh
cd m0
...
make clean
echo Signing rom
java -jar 'other/signapk.jar' 'other/testkey.x509.pem' 'other/testkey.pk8' "unsigned-miuigalaxy-v5-sgs3-$version-4.4.zip" "miuigalaxy-v5-sgs3-$version-4.4.zip"
rm -r unsigned-miuigalaxy-v5-sgs3-$version-4.4.zip

md5=`md5sum miuigalaxy-v5-sgs3-$version-4.4.zip | cut -d" " -f1`
size=`du -sh miuigalaxy-v5-sgs3-$version-4.4.zip | cut -c1-4`
data=`date +%-d/%-m/%Y`
LINK_PL="http://91.205.75.29//zdunex25/$version/miuigalaxy-v5-sgs3-$version-4.4.zip"
forum="<a href='http://bit.ly/1as1Xao'>Dyskusja na forum</a>"
MIRROR1_PL="https://s.basketbuild.com/devs/ZduneX25/miuiv5/m0/"
#MIRROR2_PL="http://htcfanboys.com/download/acid/files/MIUIv5/$version/miuigalaxy-v5-sgs3-$version-4.4.zip"
echo '[dwl producent="'samsung'" board="'m0'" tytul="Samsung&nbsp;Galaxy&nbsp;S3" android="'4.4.2'" miui="'$version'" data="'$data'" md5="'$md5'" informacje="'$forum'" status="" link="'$LINK_PL'" rozmiar="'$size'" mirror1="'$MIRROR1_PL'" mirror2="" rodzaj="'pelna'"]
    
    ' >> ../download_v5.txt

#if [ -f "miuigalaxy-v5-sgs3-$version-4.4.zip" ];
#then
#echo -e "\nPreparing OTA zip.."
...
rm miui_log.log
read -p "Done, miuigalaxy-v5-sgs3-$version-4.4.zip has been created in root of m0 directory, copy to sd and flash it!"
